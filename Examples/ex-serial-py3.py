...
class rcvSerialThread(threading.Thread):
	...
	def run(self):
		...
		while not(exitFlag):
			...
			if (intNoChars > 0):
				self.rxByteByte = self.serialPortObj.read(1)
...
if __name__ == "__main__":
	...
	mySerialPort.writeSerial(str.encode("Serial port running..."))
	...
	while not (exitFlag):
		...
		try:
			...
			# Ciclo attesa utente
			strInputBuffer = input("TX => ")
			mySerialPort.writeSerial(str.encode(strInputBuffer))
			...
			template =  "ERROR: An exception of type {0} occured." + \
						"Arguments:\n{1!r}"
			msg = template.format(type(ex).__name__, SomeErrors.args)
			print(msg)
			sys.stdout.flush()
			exitFlag = True
